...
## input data
# read count_matrix
count.all <- read.csv(paste0(root_dir, "/data/GSE141064_count.final.csv.gz"), row.names = 1) # nolint
dim(count.all)
# read meta data
...
Seu.all <- CreateSeuratObject(
  counts = count.all, meta.data = meta.all, min.cells = 2, min.features = 1,
  project = "Seu.all"
)
